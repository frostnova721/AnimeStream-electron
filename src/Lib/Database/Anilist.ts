...
import { IAnilistInfo, IAnilistInfoResult, IAnimeDetails, IAnimeSearchResult, ISeasonResponse } from '../../Types';
...
export class AniList {
    ...
    public getAnimeInfo = async(anilistId: string): Promise<IAnimeDetails> => {
        ...
        `
        const response: { Page: { media : IAnilistInfoResult[] } } = (await request('https://graphql.anilist.co', query))
        const convertToIAnimeDetails = (): IAnimeDetails => {
            ...
            const convertedGuy: IAnimeDetails = {
              ...
              aired: {
                start: `${info.startDate.day ?? ''} ${this.MonthnumberToMonthName(info.startDate.month)?.short ?? ''} ${info.startDate.year ?? ''}`,
                end: `${info.endDate.day ?? ''} ${this.MonthnumberToMonthName(info.endDate.month)?.short ?? ''} ${info.endDate.year ?? ''}`
              },
    ...
    private MonthnumberToMonthName = (monthNumber: number | string): { full: string, short: string } | undefined => {
