...
export class GogoStreams {
     ...
     * @returns 
     */
    public getStreams = async(episodeId: string, quality?: '360' | '480' | '720' | '1080' ): Promise<IStreamOutput> => {
        ...
        const encryptedData = await this.fetch(`${episodeLink.protocol}//${episodeLink.hostname}/encrypt-ajax.php?${params}`, {
            headers: {
        ...
        if(quality) {
            ...
            return { totalSources: specificArray.length, sources: specificArray, iframe: parsedData.linkiframe }
        }
        
        return { totalSources: concatedList.length, sources: concatedList, iframe: parsedData.linkiframe }
    }

    private generateQualitiesFromBaseStreamLink = async(streamLink: string):Promise<{ resolution: string, quality: string, link: string }[]> => {
        const qualityArray = []
    ...
    private getAnimeEpisodeLink = async(aliasId: string):Promise<{link: string, episodes: number}> => {
