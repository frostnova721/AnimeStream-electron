...
export class StreamWish {
    ...
    private qualityExtractor = async (m3u8Link: string) => {
        ...
        const streamDetails = splitLines.filter((line: string) =>
            line.startsWith('#EXT-X-STREAM-INF:'),
        );
        const streamLines = splitLines.filter(
            (line: string) => !line.startsWith('#EXT-X-STREAM-INF:') && !line.startsWith('#EXTM3U'),
        );

        const data: { stream: string; resolution: string; framerate: string }[] = streamDetails.map(
            (line: string, index: number) => {
                const resolution = line.match(/RESOLUTION=\d+x\d+/);
                const frameRate = line.match(/FRAME-RATE=\d+.\d+/);
                if (frameRate && resolution) {
                    return {
                        stream: `${m3u8Link.split('master.m3u8')[0]}${streamLines[index].replace(
                            '\r',
                            '',
                        )}`,
