...
document.addEventListener('DOMContentLoaded', async() => {
    ...
    for(const download of downloads) {
        container.appendChild(createDownloadItem(download.title, download.path, download.status, download.size))
    } 

})

function createDownloadItem(title: string, location: string, status: 'downloading' | 'downloaded' | 'cancelled', size: number) {
