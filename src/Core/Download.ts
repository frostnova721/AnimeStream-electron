...
interface DownloadEvents {
    ...
    kill: (killed: boolean) => boolean;
}

export declare interface Downloader {
    on<U extends keyof DownloadEvents>(event: U, listener: DownloadEvents[U]): this;

    emit<U extends keyof DownloadEvents>(event: U, ...args: Parameters<DownloadEvents[U]>): boolean;
}
